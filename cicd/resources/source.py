...
class Source:
    ...
    def __init__(
        ...
        if base64_credentials is None:
            ...
            self.headers = {
                'Authorization': 'Basic %s' % self.base64_credentials, 'Content-Type': 'application/xml',
            }
    ...
    def _list_sources(self):
        ...
        if response.status_code == 200:
            sourcelistsummary = response.text
            data_dict = xmltodict.parse(sourcelistsummary)
    ...
    def update_source(self):
        ...
        if self.source_id not in source_ids:
            raise RuntimeError(
                'Source with this ID is not Present, Please create a new source',
            )
